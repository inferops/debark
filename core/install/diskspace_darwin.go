...
// filesystem holding path. install never installs anything on macOS (there
// is no macOS target-side role at all), but this keeps `go build
// ./...` and `go vet ./...` meaningful on a macOS development machine instead
// of silently degrading to the "unsupported" stub.
