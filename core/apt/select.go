...
func localMismatchReason(ctx context.Context, hostCap Capabilities, target snapshot.Target) (string, bool) {
	if !hostCap.Available {
		reason := hostCap.Reason
		if reason == "" {
			reason = "apt-get is not available on this host"
		}
		return reason, true
	}
	if target.DistroID != "" && hostCap.DistroID != "" && !strings.EqualFold(hostCap.DistroID, target.DistroID) {
		return fmt.Sprintf("host distro %q does not match target distro %q", hostCap.DistroID, target.DistroID), true
	}

	return solverGateReason(hostSolverProbeFunc(ctx), hostCap.APTVersion, target)
}

func localSummary(hostCap Capabilities) string {
	return fmt.Sprintf("%s %s, apt %s", hostCap.DistroID, hostCap.VersionID, hostCap.APTVersion)
}
...
func localSelectedReason(base string, hostCap Capabilities, target snapshot.Target) string {
	if note := aptVersionDivergenceNote(hostCap.APTVersion, target.APTVersion); note != "" {
		return base + "; " + note
	}
	return base
}
