...
| Valid, marker and index disagree, nothing in flight, and the disagreement **does not survive a second read** | `ErrCacheBusy`. The commit finished between the load's samples; **never delete**. |
| Integrity failure in `catalog.bin`, or a disagreement that **reproduces from a directory that did not move between the two reads**, with no commit in flight | `ErrCacheCorrupt`. Delete the directory, offer to build. |
...
- **The two renames are one commit, and step 5 losing that caveat is the one
  ...
  restore are attempted in turn until one lands. If neither does inside the
  budget, **nothing is removed**: the marker stays, and so does the link, which
  both holds the index the marker describes and is what tells a reader this
  directory is mid-commit. The rule is not "remove nothing" but *never leave a
  quiet mismatch* — a disagreement with no sign beside it is the one state a
  reader may delete the directory over — so where there was nothing to link
  aside, the marker comes off instead. See §10.
...
- **§7 does not cover `meta.json` failing to write after `catalog.bin` was
  ...
  describes instead. Removing the marker when *neither* direction could be made
  to land was kept as a last resort, and that was still the same bug in a
  narrower window: a rebuild racing four readers reached it about once in sixty
  runs of `TestCacheReaderRacingWriter`, and every reader then saw an empty
  directory over a catalogue still whole on disk under its hard link. Nothing
  is removed there now — the link stays as both the surviving index and the
  "commit in flight" sign, which is what makes the disagreement a reader then
  sees `ErrCacheBusy`. A quiet mismatch is never left behind: with no link to
  leave, the marker still comes off, because that costs the cache rather than
  the whole directory.
- **§5's "delete it and rebuild" is wrong for a `meta.json`/`catalog.bin`
  ...
  Those three signs are still not enough, because each is one sample taken
  after the fact and none of them brackets the window the disagreement was read
  in. Windows CI found the interleaving that walks between all three: the first
  rename lands *before* the load stats `catalog.bin`, so the index looks
  untouched at both ends; the marker is re-read while the second rename is
  still to come, so the marker looks untouched too; and the second rename then
  lands, temporaries and all, before the load lists the directory. Three honest
  signs, all blind, over a commit that was completing the whole time — verdict
  `ErrCacheCorrupt`, and the directory deleted. **A disagreement is therefore
  believed only when it reproduces**: the same marker bytes, the same index
  stamp and the same failure, read twice. Damage does that every time; a commit
  the load straddled does not, because by the second read it has landed and the
  pair agrees. Evidence that keeps changing instead is itself the writer none
  of the three signs caught, and answers `ErrCacheBusy`.
