...
// and what it protects — a directory left with a new index under a stale
// marker — is the state this whole function exists to get out of.
//
...
// shows any sign of being written *or* keeps changing under the look. It has
// to outlast a whole commit — every attempt, and the undo after each — because
// the alternative to waiting is deleting a cache that was never damaged. It is
// paid only by a load that has already failed, and only while something is
...
//
// All three are point samples, and none of them brackets the window the
// disagreement was actually read in, so a commit can pass clean through the
// middle of them. Windows CI caught exactly that, and the interleaving is worth
// writing down because every step of it is ordinary:
//
//	writer renames catalog.bin          (phase 1 lands)
//	reader stats catalog.bin            -> the new index, and it never moves again
//	reader reads meta.json              -> still the *old* marker
//	reader reads catalog.bin            -> the new index: 700 rows under a marker saying 300
//	reader re-reads meta.json           -> still the old marker; phase 2 has not run yet
//	writer renames meta.json            (phase 2 lands, and the temporaries go)
//	reader lists the directory          -> nothing in flight
//
// Three honest signs, all blind, over a commit that was completing the whole
// time. The verdict was ErrCacheCorrupt and the directory was deleted.
//
// So the disagreement is never itself the evidence. meta.json and catalog.bin
// are read at two different instants and each is independently
// integrity-checked on its own (JSON parse, and the format's CRC and bounds),
// so a disagreement says the two reads landed in different generations and
// nothing whatever about either file being damaged. What separates damage from
// a race is that damage *stays*: a directory nothing is writing hands back the
// same marker bytes, the same index stamp and the same failure however many
// times it is read, and a straddled commit does not. A quiet disagreement is
// therefore read again, and only a failure that reproduces from a demonstrably
// unchanged directory is believed. That costs one extra read on a cache that
// really is damaged and saves the one that is not — and it is also what covers
// the marker re-read reporting "unmoved" when it could not be read at all.
//
// So the directory is only ever removed on a failure that reproduced against a
// directory nothing was touching. While one of the three signs holds, or the
// evidence keeps changing between two looks that both saw a quiet directory,
// the load is retried for cacheRacePatience; after that the answer is
// ErrCacheBusy and the bytes are left exactly where they are, because a caller
// that is told to rebuild loses a download and a caller whose cache was deleted
// loses it too — but the second one cannot be taken back. This is the whole
// concurrency protocol: no lock file, no lock protocol, just atomic renames, a
// re-read, a failure that has to happen twice, and never deleting what somebody
// else is writing.
func LoadCacheDir(dir string) (*CacheFile, error) {
	patientUntil := time.Now().Add(cacheRacePatience)
	var previous cacheVerdict
	repeatable := false
	for attempt := 0; ; attempt++ {
		...
		after := cacheStatIndex(dir)
		changed := cacheMarkerMoved(dir, marker) || !after.sameAs(before)
		writing := changed || cacheCommitInFlight(dir)

		// A foreign format version is decided from the marker alone. No amount
		// of waiting changes it and reading it twice cannot either, so it is
		// reported at once — but it is still not deleted out from under a
		// build that is mid-commit.
		...
		if writing && time.Now().Before(patientUntil) {
		...
		// Nothing looks like it is writing — and "looks like" is three samples
		// taken after the fact, any of which a commit can finish between. So
		// the failure has to happen twice, from a directory that is provably
		// the same one both times. Damage does that on every read; a commit
		// this load straddled does not, because by now it has landed and the
		// pair agrees.
		this := cacheVerdict{marker: marker, index: after, failure: err.Error()}
		if !repeatable || !this.sameAs(previous) {
			if !time.Now().Before(patientUntil) {
				// The evidence never settled, which is itself the writer none
				// of the three signs caught. Busy, and nothing is removed.
				return nil, fmt.Errorf("%w: %s: %v", ErrCacheBusy, dir, err)
			}
			if repeatable {
				// It moved between two looks that both saw a quiet directory.
				// Give the writer the same 50 ms the other retry path does
				// rather than re-reading a 19 MB index straight away.
				time.Sleep(cacheCommitWait)
			}
			previous, repeatable = this, true
			continue
		}
		if errors.Is(err, ErrCacheCorrupt) || errors.Is(err, ErrNotBuilt) {
			cacheRemoveDir(dir)
		}
		return nil, err
	}
...
// cacheVerdict is what one pass of LoadCacheDir's loop had in front of it: the
// commit marker's bytes, the stamp catalog.bin carried at the end of the read,
// and what the cross-checks made of the pair.
//
// Two passes that agree on all three read the same directory twice and got the
// same answer out of it. That, and not the absence of a temporary file, is what
// makes a marker/index disagreement damage rather than a commit that slipped
// between this load's samples.
//
// The failure is compared as its message because the message *is* the evidence
// — "meta.json says 300 entries, catalog.bin holds 700" names both halves of
// the disagreement — and because two different failures from a directory that
// looks unchanged mean the look is wrong, which is the case that must not be
// believed. The zero value matches nothing: an empty message is not one a
// failure can carry.
...
func (v cacheVerdict) sameAs(o cacheVerdict) bool {
	return v.failure == o.failure && v.index.sameAs(o.index) && bytes.Equal(v.marker, o.marker)
}
...
// it and carries on rather than throwing away a completed build. A commit that
// could not be finished *or* undone is the one case that leaves a temporary
// behind on purpose; cacheCommitPair says why.
...
//
// If neither direction lands inside the budget, the marker stays and so does
// the hard link. Removing the marker is what this did first, on the grounds
// that "not built" is at least true where "corrupt" would be a lie — but it is
// a lie of its own, and an expensive one: the previous catalogue is still on
// disk and still whole, and every window with that cache open reads the
// directory as empty until some later build happens to win the same race.
// Windows CI caught it in the same test as the reader's half of this.
//
// The link is both the copy of the index the marker describes and the sign
// that tells a concurrent reader this directory is mid-commit, which is what
// makes the disagreement that reader is about to see ErrCacheBusy rather than
// ErrCacheCorrupt. So the rule here is not "remove nothing" but *never leave a
// quiet mismatch*: a mismatch with no sign beside it is the one state a reader
// may call damage and delete the whole directory over. When there was nothing
// to link aside — no previous cache, or a filesystem that would not link one —
// the marker does come off, because then there is no sign to leave and "not
// built" costs the cache that is lost either way rather than the directory.
//
// The next build resolves the directory. If none comes, the sweep collects the
// link an hour later and what is left really is a quiet mismatch with no way
// back, which is exactly when a reader should delete it.
//
...
func cacheCommitPair(binTmp, binPath, metaTmp, metaPath string) error {
	...
	// Neither direction landed, so the directory keeps the new index under the
	// previous marker. Leave the link beside them: it holds the index the
	// marker describes and it is what makes the disagreement a reader is about
	// to see ErrCacheBusy. With no link there is no sign to leave, and a quiet
	// mismatch is the one thing a reader may delete the directory over, so the
	// marker comes off instead. See the note above.
	if rollback == "" {
...
// mid-commit — or by a commit that could be neither finished nor undone, which
// keeps it deliberately — is collected like every other leftover temporary,
// and, while it is there, it is also what tells a concurrent reader that this
// directory is being written and must not be deleted.
