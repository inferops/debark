...
> **Pre-1.0.** `v0.1.0` is tagged: prebuilt Linux and Windows binaries,
> `.deb` and `.rpm` packages are on the [releases
> page](https://github.com/inferops/debark/releases), and `go install` or one
> `go build` from a checkout work too (see [Install](#install)). Before 1.0.0
> a minor version may include breaking changes. The pipeline below is real
> and runs end to end today. [Status and known
> gaps](#status-and-known-gaps) lists what isn't finished.
...
**`v0.1.0` is the first tagged release.** The pipeline that produces it is
gated on a reproducible-build check (`hack/reproducible-check.sh`), and every
file on that page is listed in the signed checksums file described above, so
a download can be checked before it is trusted. The two channels below build
the binary yourself instead, which is the right choice on any platform this
project does not publish an artefact for.
...
- **Only one version has been tagged, and the distribution channels are
  thin.** `v0.1.0` publishes a single GitHub release carrying the CLI
  archives (`linux_amd64`, `linux_arm64`, `windows_amd64`), `.deb` and
  `.rpm` packages, cosign-signed checksums and an SBOM, alongside the
  desktop application from `gui/`. That release, `go install` and a source
  build are the three channels that work. Still intended and not built: a
  ...
  not any more: no macOS artefact is published for one to point at. SLSA
  build provenance is wired but does not run while this repository is
  private — the generator refuses, to keep a private repository's name out
  of the public transparency log.
