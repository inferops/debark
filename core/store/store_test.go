...
func TestPutFileStoresOnlyBytesItHashed(t *testing.T) {
	...
	seen := map[string]bool{}
	// The invariant, checked identically in both phases below: whatever
	// PutFile stored must hash to the address PutFile reported for it.
	ingest := func() {
		dg, _, err := st.PutFile(ctx, src, false)
		if err != nil {
			return // a torn read may fail; storing wrong bytes may not
		}
		seen[dg] = true
	...
	// Phase 1 - the concurrent window, which is the thing that can actually
	// catch PutFile storing bytes it did not hash. Whether the rewriter
	// interleaves inside any given PutFile is the scheduler's business, so
	// nothing is concluded here from how many distinct digests it produced.
	for i := 0; i < 120; i++ {
		ingest()
	}
	stopRewriter()

	// Phase 2 - the same invariant with the source rewritten under this
	// goroutine's own control. Its job is to make the guard below mean
	// something on every machine: a and b hash differently, so two distinct
	// digests are ingested here regardless of scheduling.
	//
	// Phase 1 used to carry that guard alone, and it was flaky exactly as you
	// would expect - CI hit "1 distinct digests ingested" on a quiet runner
	// while passing on a busy one. A test that fails when a race it cannot
	// command does not materialise reports the runner, not the product.
	...
	// Without this the test would pass having ingested nothing at all - if
	// every PutFile above returned an error, seen stays empty and no
	// assertion in ingest ever ran.
	if len(seen) < 2 {
		t.Fatalf("only %d distinct digest(s) ingested; this test asserted nothing", len(seen))
	}
}
